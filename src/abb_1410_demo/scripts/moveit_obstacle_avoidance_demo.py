...
class MoveItObstacleAvoidanceDemo:
    def __init__(self):
        ...
        # 获取规划器参数并设置规划尝试次数
        self.arm.set_planning_time(5.0)  # 设置规划时间
        self.arm.set_num_planning_attempts(10)  # 设置规划尝试次数
    ...
    def create_obstacle_environment(self):
        ...
        self.add_cylinder("cylinder_obstacle_2", 
                        [0.9, 0, 0.95], 
                        0.6, 0.06)
        
        
        rospy.loginfo("已创建简化的障碍物环境（仅包含柱形障碍物）")
    ...
    def obstacle_avoidance_demo(self):
        ...
        try:
            for i, target_pose in enumerate(target_poses):
                ...
                # 规划运动路径
                rospy.loginfo(f"正在规划到目标位姿 {i+1}，尝试避开障碍物...")
                plan = self.arm.plan()
                
                # 检查规划是否成功
                if plan and isinstance(plan, tuple) and plan[0]:
                    traj = plan[1]  # 提取轨迹部分
                    rospy.loginfo(f"到目标位姿 {i+1} 的规划成功!")
                    
                    # 打印规划点数量
                    if hasattr(traj, 'joint_trajectory') and hasattr(traj.joint_trajectory, 'points'):
                        points_count = len(traj.joint_trajectory.points)
                        rospy.loginfo(f"规划的轨迹包含 {points_count} 个点")
                    
                    # 按照规划的运动路径控制机械臂运动
                    self.arm.execute(traj)
                    rospy.sleep(1)
                else:
                    rospy.logerr(f"到目标位姿 {i+1} 的规划失败，可能无法避开障碍物")
                    # 尝试增加规划时间和尝试次数
                    self.arm.set_planning_time(10.0)
                    self.arm.set_num_planning_attempts(20)
                    
                    # 重新规划
                    rospy.loginfo("增加规划参数，重新尝试...")
                    plan = self.arm.plan()
                    
                    if plan and isinstance(plan, tuple) and plan[0]:
                        traj = plan[1]
                        rospy.loginfo("重新规划成功!")
                        self.arm.execute(traj)
                        rospy.sleep(1)
                    else:
                        rospy.logerr("重新规划仍然失败，跳过该目标")
                        continue
            ...
            rospy.logerr(f"避障演示过程中发生错误: {e}")


if __name__ == "__main__":
    demo = MoveItObstacleAvoidanceDemo()
